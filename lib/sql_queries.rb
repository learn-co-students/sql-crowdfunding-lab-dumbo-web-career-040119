...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS amount_to_goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY pledges.project_id HAVING amount_to_goal >= 0";
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) AS total_amount FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY pledges.user_id ORDER BY total_amount, users.name";
end
